...
HRESULT CLoopbackCapture::StartCaptureAsync(DWORD processId, bool includeProcessTree)
{
    ...
    if (m_DeviceState == DeviceState::Initialized)
    {
        ...
        auto result = MFPutWorkItem2(MFASYNC_CALLBACK_QUEUE_MULTITHREADED, 0, &m_xStartCapture, nullptr);
...
//void CLoopbackCapture::AddAudioCallback(AudioCallback callback){
//    this->m_audioCallbacks.push_back(callback);
//}

AudioData* CLoopbackCapture::GetNextAudioData(){
    return this->m_audioDataQueue.pop();
}
...
//    FixWAVHeader();
    ...
    m_audioDataQueue.push(&m_emptyData);
    return S_OK;
}
...
//                &dwBytesWritten,
//                NULL));
            BYTE* copy = new BYTE[cbBytesToCapture];
            memcpy(copy, Data, cbBytesToCapture);
            auto* data = new AudioData();
//            std::cout  << "Client received audio data: " << static_cast<int>(cbBytesToCapture) << std::endl;
            data->data = copy;
            data->length = cbBytesToCapture;
            m_audioDataQueue.emplace(data);
        }
...
CLoopbackCapture::CLoopbackCapture(CLoopbackCapture const &capture) {
